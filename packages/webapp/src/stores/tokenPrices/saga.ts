...
const getTokenPricesApi = async <R extends { [ key: string ]: any }>() => {
    ...
    if(LoopringAPI?.walletAPI && addressIndex) {
        const {tokenPrices:_tokenPrices,raw_data} = await LoopringAPI?.walletAPI.getLatestTokenPrices()
        let {__timer__} = store.getState().tokenPrices;
        const tokenPrices:{[key in keyof R]:number} = Reflect.ownKeys(_tokenPrices).reduce((prev, address) => {
            const symbol = addressIndex[ address as string ];
...
export const tokenPricesSaga = [
    fork(tokenPricesInitSaga),
]

...
export function useChargeNFTFees({ tokenAddress,tokenMap, requestType, amount, needRefresh, }:
{   tokenAddress:string|undefined,
    tokenMap: LoopringMap<TokenInfo>,
    requestType: OffchainNFTFeeReqType,
    amount?: number, needRefresh?: boolean
}) {
    ...
    const getFeeList = React.useCallback(
        _.debounce(async (accountId: number, apiKey: string, tokenAddress: string | undefined, requestType: OffchainNFTFeeReqType,
                          tokenMap: LoopringMap<TokenInfo> | undefined, amount: number | undefined) => {

            if (accountId === -1 || !apiKey || !tokenAddress || !LoopringAPI.userAPI || !tokenMap) {
                return
            }
            ...
            try {
                ...
                const response = await LoopringAPI.userAPI.getNFTOffchainFeeAmt(request, apiKey)
    ...
    }, [needRefresh, account.accountId, account.apiKey, tokenAddress, requestType, amount])

...
export const useSwap = <C extends { [key: string]: any }>({
  ...
  const smallOrderAlertCallBack = React.useCallback((confirm: boolean) => {
    if (confirm) {
      swapFunc();
      setIsSwapLoading(false)
  ...
  const secondConfirmationCallBack = React.useCallback((confirm: boolean) => {
    if (confirm) {
      swapFunc();
  ...
  React.useEffect(() => {
    ...
    accountStatus,
    isSwapLoading,
    pageTradeLite.calcTradeParams?.amountBOut,
    sellMinAmt,
  ...
  const reCalculateDataWhenValueChange = React.useCallback(
    (_tradeData, _tradePair?, type?) => {
      ...
      if (depth && market && _tradePair === tradePair) {
        ...
        myLog("updatePageTradeLite feeBips:", feeBips, _tradeCalcData);

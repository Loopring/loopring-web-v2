...
export const useAmmExit = <C extends { [key: string]: any }>({
    ...
    const { account, status: accountStatus, account: { accountId, apiKey } } = useAccount();
    ...
    }, [snapShotData, coinMap, tokenMap, ammCalcData, ammMap, setAmmCalcData, setAmmExitData])
    ...
    const removeBtnLabelActive = React.useCallback(({ammExitData}): string | undefined => {
        ...
        if (isExitLoading) {
            setAmmWithdrawBtnI18nKey(TradeBtnStatus.LOADING)
            return undefined
        } else {
            if (account.readyState === AccountStatus.ACTIVATED) {
                if (true) {
    ...
    const calculateCallback = React.useCallback(async ()=>{
        if (accountStatus === SagaStatus.UNSET) {
            ...
            myLog('-> feeExit:', feeExit)

            setAmmCalcData({ ...ammCalcData, feeExit })
        }

    },[
        setExitFees, setAmmCalcData, setAmmWithdrawBtnI18nKey,

...
export const useAmmJoin = ({
    ...
    const handleJoin = React.useCallback(async ({data, type, fees, ammPoolSnapshot, tokenMap, account}) => {

        if (!data || !tokenMap || !data.coinA.belong
            || !data.coinB.belong || !ammPoolSnapshot) {

import { AccountStatus, NFTWholeINFO, SagaStatus } from '@loopring-web/common-resources'
...
export const useMyNFT = () => {
    ...
    const onDetail = React.useCallback(async (item: Partial<NFTWholeINFO>) => {
       const nftData:NftData = item.nftData as NftData ;
       const nftMap =  await  LoopringAPI?.nftAPI?.getInfoForNFTTokens({nftDatas:
                // [item.tokenAddress]
            [nftData]
        })
        const nftToken:Partial<NFTTokenInfo> = (nftMap && nftMap[nftData as NftData]) ?nftMap[nftData as NftData]:{}

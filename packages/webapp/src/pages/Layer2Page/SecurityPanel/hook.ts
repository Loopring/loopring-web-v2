...
export function useExportAccountInfo() {
    ...
    const exportAccount = React.useCallback(async () => {
        ...
        if (exchangeInfo && LoopringAPI.userAPI && account.nonce !== undefined) {
            try {
                setShowAccount({ isShow: true, step: AccountStep.ExportAccount_Approve_WaitForAuth })
                ...
                if (!apiKey && raw_data.resultInfo) {
                    ...
                    setShowAccount({ isShow: true, step: AccountStep.ExportAccount_Failed })
                } else {
                ...
                switch (errType) {
                    case sdk.ConnectorError.USER_DENIED:
                        setShowAccount({ isShow: true, step: AccountStep.ExportAccount_User_Denied })
                        return
                    default:
                        break
                }

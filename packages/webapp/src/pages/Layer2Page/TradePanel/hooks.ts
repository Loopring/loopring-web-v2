...
import { Side, toBig, GetUserTradesRequest, MarketTradeInfo } from '@loopring-web/loopring-sdk'
...
export function useGetTrades() {
    ...
    const getUserTrade = React.useCallback(async(props?: Omit<GetUserTradesRequest, 'accountId'>) => {
    ...
    const getUserOrderDetailTradeList = React.useCallback(async(props?: Omit<GetUserTradesRequest, 'accountId'>) => {
        if (LoopringAPI && LoopringAPI.userAPI && accountId && apiKey && tokenMap) {
            const userTrades = await getUserTrade({...props})
            if (userTrades && userTrades.userTrades) {
                setUserOrderDetailList(userTrades.userTrades.map(o => {

  ...
  return (
    <div class="MapContainer">
      ...
      <footer class="MapFooter">
        <h2>History</h2>
        <p>
          Current:. Floor ${y}. Node ${x}
        </p>
        <ul>{pathTaken.map((path) => html`<li>${path.y}.${path.x}</li>`)}</ul>
      </footer>
    </div>
  );
};
...
export class SlayMap extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    this.base.style.setProperty(
      "--rows",
      Number(prevProps.dungeon.graph.length)
    );
    this.base.style.setProperty(
      "--columns",
      Number(prevProps.dungeon.graph[1].length)
    );
  ...
  drawPath(path, preferredIndex) {
    ...
    const elFromNode = ([row, col]) =>
      containerElement.childNodes[row].childNodes[col];
  ...
  render(props) {
    ...
    if (!dungeon.graph)
      throw new Error("No graph to render. This should not happen?", dungeon);
          ...
          (row, rowIndex) => html`
            <slay-map-row current=${rowIndex === y}>
              ${row.map((node, nodeIndex) => {
                ...
                const completedCurrentRoom = isRoomCompleted(
                  dungeon.graph[y][x].room
                );

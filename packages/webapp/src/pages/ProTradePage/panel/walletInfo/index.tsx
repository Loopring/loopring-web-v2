...
const OtherView = React.memo(({t}: { market: MarketType, t: TFunction }) => {
    ...
    const BtnConnect = React.useMemo(() => {
        return <Button style={{height: 28, fontSize: '1.4rem'}} variant={'contained'} size={'small'} color={'primary'}
                       onClick={() => {
    ...
    return <Box paddingBottom={2}>
        ...
        <Box paddingX={2} display={'flex'} flex={1} flexDirection={'column'} justifyContent={''}>
            ...
            <Box display={'flex'} flexDirection={'row'} alignItems={'center'} marginTop={2} justifyContent={'center'}>
                <Box marginRight={1}>
                    <Button style={{height: 28, fontSize: '1.4rem'}} variant={'contained'} size={'small'}
                            color={'primary'} onClick={() => onShowDeposit(coinA)}>{t('labelDeposit')}</Button></Box>
                <Box marginLeft={1}><Button style={{height: 28, fontSize: '1.4rem'}} variant={'outlined'} size={'small'}
                                            onClick={() => onShowTransfer(coinA)}>{t('labelTransfer')}</Button></Box>
            </Box>

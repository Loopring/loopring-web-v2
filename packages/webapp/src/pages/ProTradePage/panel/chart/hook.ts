...
export function useKlineChart(market: string | undefined) {
  ...
  const [candlestickViewData, setCandlestickViewData] = React.useState<
    IOHLCData[]
  >([]);
  const {
  ...
  const addCandlestick = React.useCallback(() => {
    if (market) {
      ...
      const tradeArray =
        store.getState()._router_pageTradePro.pageTradePro.tradeArray;
      setCandlestickViewData((candlestickViewData) => {
        if (tradeArray && candlestickViewData.length) {
          const viewDate =
            candlestickViewData[candlestickViewData.length - 1].date;
          ...
          let index =
            tradeArray.findIndex((item) => {
              ...
              return (
                moment(item.time).diff(moment(lastMinutes), "millisecond") < 0
              );
            }) - 1;
          ...
          if (
            moment(now).diff(moment(viewDate), "millisecond") >
            TradingIntervalToTimer[timeInterval]
          ) {

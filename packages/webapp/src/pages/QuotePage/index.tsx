    ...
    <RowStyled container spacing={2}>
      ...
      {(!!recommendations.length ? recommendations : [{
        ...
        [ key: string ]: string;
          ...
          <Grid key={`${item.coinAInfo.simpleName}-${item.coinBInfo.simpleName}-${index}`} item xs={12} sm={6}
                lg={3}>
            <MarketBlock {...{
    ...
    <TableWrapStyled ref={tableRef as any}
      ...
      <Box display={'flex'} flexDirection={'column'}>
        <Container className={'toolbar'}>
          <Box paddingLeft={1} paddingRight={2} display={'flex'} flexDirection={'row'} justifyContent={'space-between'}
               alignItems={'center'}>
            <Tabs
              value={tableTabValue}

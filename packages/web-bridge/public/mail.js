...
var zsFieldsDisplayLabelArray = ["Last Name", "Email", "Subject"];
...
function zsValidateMandatoryFields() {
  ...
  for (var index = 0; index < zsWebFormMandatoryFields.length; index++) {
    isError = 0;
    var fieldObject =
      document.forms["zsWebToCase_638510000000180873"][
  ...
  if (isError == 0) {
    document
      .getElementById("zsSubmitButton_638510000000180873")
...
function zsOpenCloudPickerIframe() {
  if (zsAttachedAttachmentsCount < 5) {
    ...
    jQuery("#zsCloudAttachmentIframe")
      .attr(
        "src",
        zsCloudPickerIframeSrc.substring(0, zsCloudPickerIframeSrc.length - 1) +
        ...
        for (
          ...
          if (zsAttachedAttachmentsCount <= zsAllowedAttachmentLimit) {
            ...
            zsCloudAttachmentsList =
              ...
              '<a id="' +
              ZSEncoder.encodeForHTMLAttribute(zsCloudAttachmentId) +
              '" cloudservice="' +
    ...
    for (
      ...
      if (isZsCloudAttachmentRemoved != 1) {
        jQuery.each(
          ...
          function (attachmentsDetailJsonKey, attachmentsDetailJsonValue) {
            if (
              attachmentsDetailJsonKey == "docId" &&
              attachmentsDetailJsonValue.replace(/\s/g, "") == cloudAttachmentId
            ) {
...
function zsRenderBrowseFileAttachment(zsAttachmentObject, zsAttachmentDetails) {
  if (zsAttachmentObject != "") {
    ...
    if (zsAttachedAttachmentsCount < 5) {
      ...
      var zsCurrentAttachmentIdTokens = jQuery(zsAttachmentDetails)
        .attr("id")
        .split("_");

...
import { accountStaticCallBack, btnClickMap, useAccount, useDualMap, useTokenPrices } from '@loopring-web/core'
...
export const EarnPage = withTranslation('common', { withRef: true })(() => {
  ...
  const buyDipTokens: {
    ...
    tokenList?.map((token) => {
      const keys = Object.keys(marketMap).filter((key) => key.includes(token.tokenName))
      const maxApy = max(keys.map((key) => (marketMap[key] as any).quotaTokenApy?.max as number))
      return {
        symbol: token.tokenName,
        apy:
          toBig(maxApy ?? 0)
  ...
  return (
    ...
    >
      ...
      >
        ...
        :(
          <ConnectBtn onClick={() => {
            ...
            <Typography variant={'h2'}>Unlock</Typography>
          </ConnectBtn>
        )}
      ...
      <Box marginTop={12.5} display={'flex'} flexWrap={'wrap'}>
        {[...sellCoverTokens, ...buyDipTokens].map((info, index) => {
          return (
            <EarnCard key={info.symbol} marginRight={index % 3 === 2 ? '0' : '2%'}>
              ...
              <Button onClick={() => {
                history.push(`/invest/dual?viewType=${info.tag === 'buyDip' ? 'DualDip' : 'DualGain'}&autoChose=${info.symbol}`)

...
const getWalletL2NFTCollectionBalance = async <
...
>({
  ...
  if (apiKey && accountId && LoopringAPI.userAPI) {
    ...
    collections = (response as any).raw_data?.collections?.map((item: any) => {
      return {
        ...item.collection,
        count: item.count,
      };
...
export function* getPostsSaga({ payload: { page = 1 } }: any) {
  try {
    // @ts-ignore
    const walletL2NFTCollection: any = yield call(
      getWalletL2NFTCollectionBalance,
      {
        page,
      }
    );
    ...
    yield put(getWalletL2NFTCollectionStatus(err));
  }
}

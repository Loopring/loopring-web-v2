...
export const useBtradeMap = (): BtradeMapStates & {
  ...
  const btradeMap: BtradeMapStates = useSelector(
    (state: RootState) => state.invest.btradeMap
  );
  ...
  return {
    ...
    getBtradeMap: React.useCallback(
      () => dispatch(getBtradeMap(undefined)),
      [dispatch]
    ),
    updateBtradeSyncMap: React.useCallback(
      ({ btradeMap }: { btradeMap: BtradeMap }) =>
        dispatch(getBtradeMap(btradeMap)),
      [dispatch]
    ),
  };
};

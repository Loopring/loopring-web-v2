...
export const useStakingMap = (): StakingMapStates & {
  ...
  const stakingMap: StakingMapStates = useSelector(
    (state: RootState) => state.invest.stakingMap
  );
  ...
  return {
    ...
    getStakingMap: React.useCallback(
      () => dispatch(getStakingMap(undefined)),
      [dispatch]
    ),
    updateDefiSyncMap: React.useCallback(
      ({ stakingMap }: { stakingMap: StakingMap }) =>
        dispatch(getStakingMap(stakingMap)),
      [dispatch]
    ),
  };
};

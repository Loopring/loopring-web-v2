  ...
  reducers: {
    ...
    updateNFTRefreshHash(
      ...
      if (state[chainId].nftDataHashes[nftData.toLowerCase()]) {
        ...
        const _txInfo: Required<TxInfo> =
          state[chainId].nftDataHashes[nftData.toLowerCase()];
...
export const { clearAll, clearNFTRefreshHash, updateNFTRefreshHash } =
  NFTHashInfoSlice.actions;

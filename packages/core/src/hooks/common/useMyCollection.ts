...
export const useMyCollection = <C extends CollectionMeta>(filter?: L2CollectionFilter): CollectionListProps<C> => {
	const [collectionList, setCollectionList] = React.useState<C[]>([]);
	const [copyToastOpen, setCopyToastOpen] = React.useState({isShow: false, type: "json"});

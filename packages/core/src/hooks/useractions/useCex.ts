      ...
      if (
        ...
        const request: sdk.OriginCEXV3OrderRequest = {
          ...
          sellToken: {
            ...
            volume: tradeCalcData?.amountS,
          },
          buyToken: {
            tokenId: buyToken?.tokenId ?? 0,
            volume: tradeCalcData?.amountB,
          },
        ...
        const info = {
          ...
          sellStr:
            getValuePrecisionThousand(
              sdk.toBig(tradeCalcData?.amountS),
              sellToken.precision,
              sellToken.precision,
              sellToken.precision,
              false,
          ...
          buyStr:
            getValuePrecisionThousand(
              sdk.toBig(tradeCalcData?.amountB),
              buyToken.precision,
              buyToken.precision,
              buyToken.precision,
              false,
  ...
  const callPairDetailInfoAPIs = React.useCallback(async () => {
    if (market && LoopringAPI.defiAPI) {
      try {
        ...
        myLog("useCexSwap:", error, "go to LRC-ETH");
      ...
      if (depth && market && _tradePair === tradePair) {
        ...
        if (tradeCost && calcDexOutput) {
          minimumReceived = sdk
            ...
            .div("1e" + buyToken.decimals)
            .toString();
          const outputType = isAtoB ? "buy" : "sell";
          _tradeData[outputType].tradeValue = calcDexOutput[`${outputType}Vol`];

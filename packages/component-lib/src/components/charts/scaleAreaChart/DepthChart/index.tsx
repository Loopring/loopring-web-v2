...
const DepthChart = withTranslation('common')(({
    ...
    const renderTooltipContent = useCallback((props) => {
        ...
        if (props.payload && !!props.payload.length && props.payload[ 0 ].payload) {
            ...
            if (payload.bids || payload.asks) {
                return (
                    <TooltipStyled>
                        <Typography>{t('labelProOrderPrice')}: {formatAxisTicker(price, true)}</Typography>
                        <Typography>{t('labelProOrderTotalAmount')}: {getValuePrecisionThousand(payload.bids || payload.asks, undefined, undefined, marketPrecision, false)}</Typography>
                    </TooltipStyled>
                )
            }
            // if (payload.asks) {
    ...
    const formatAxisTicker = React.useCallback((value: any, addZeros: boolean) => {

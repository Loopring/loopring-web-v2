...
export function KlineChart({ data: initialData, height, dateTimeFormat = "%d %b", width, ratio, }: KlineChartProps) {
    ...
    const ema12 = ema()
        ...
        const candleStickColor = React.useCallback((data: IOHLCData) => {
            return data.close > data.open ? CandleStickFill.up : CandleStickFill.down
        }, [])
    ...
    return (
        ...
        <Chart id={1} padding={{top: 10, bottom: 20}}>
            ...
            <CandlestickSeries fill={candleStickColor}/>
            <MovingAverageTooltip
                origin={[8, 24]}
                textFill={'#FFF'}
                options={[

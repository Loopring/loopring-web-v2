...
export const IconColumn = React.memo(
  <R extends AmmDetail<T>, T>({ row }: { row: R }) => {
...
const columnMode = <R extends Row<T>, T>(
  { t }: WithTranslation,
  getPopoverState: any,
  coinJson: any,
  tokenPrices: any,
...
): Column<R, unknown>[] => [
  {
    ...
    formatter: ({ row }: FormatterProps<R, unknown>) => {
      return (
        <Box flex={1} height={"100%"} alignContent={"center"} display={"flex"}>
  ...
  },
  {
    ...
    formatter: ({ row }) => {
      ...
      const totalAmountDollar =
        (Number(volume) || 0) * (tokenPrices[row.coinAInfo?.simpleName] || 0);
        ...
        <Box className={"textAlignRight"}>
          <Typography component={"span"}>
            ...
            {volume && Number.isFinite(volume)
              ...
              : EmptyValueTag}{" "}
            {/* {row.tradeFloat && row.tradeFloat.volume ? row.coinAInfo.simpleName : ''} */}
          </Typography>
        </Box>
      );
    },
...
export const PoolsTable = withTranslation("tables")(
  <T extends { [key: string]: any }>({
    ...
    return (
      <TableStyled flex={1} flexDirection={"column"} display={"flex"}>

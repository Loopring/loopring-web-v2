...
const generateRows = <Row, SR = unknown>(
  rawData: Row[],
  _rest: TableProps<Row, SR>
): Row[] => {
...
export const TradeFilterTable = withTranslation("tables")(
  ({
    ...
    const columnMode: readonly Column<Row, unknown>[] = [
          ...
          <>
            ...
            <Typography component={"p"} variant={"body2"}>
              {PriceTag[CurrencyToTag[currency]] +
                row.priceDollar * forexMap[currency]}
            </Typography>
          </>
        ),
      },
      {
        key: "change24",
        name: "labelQuota24hChange",
        sortable: true,
        cellClass: (row: Row) => `float-${row.floatTag}`,
        formatter: ({ row }: FormatterProps<Row, unknown>) => {
          return (
            <Button
              className={"btn-change24"}
              variant={"contained"}
              size={"small"}
              color={"primary"}
            >
        ...
        switch (sortColumn) {
          ...
          case "change24":
            sortedRows = sortedRows.sort(
              (a: any, b: any) => Number(a[sortColumn]) - Number(b[sortColumn])
            );
            break;
          default:
        }

...
export const CexSwapTable = withTranslation(["tables", "common"])(
  <R extends RawDataCexSwapsItem>(props: CexSwapsTableProps<R> & WithTranslation) => {
      ...
      (): Column<R, unknown>[] => [
        {
          ...
          formatter: ({ row }: FormatterProps<R, unknown>) => {
            ...
            return <Box display={"flex"} justifyContent={"space-between"} paddingRight={3}>
              <Box color={found ? found[1] : ''}>
                {row?.type ?? EmptyValueTag}
              </Box>
              <Box>
                {row ? <>{`${row.fromAmount} ${row.fromSymbol} -> ${row.toAmount} ${row.toSymbol}`} </> : EmptyValueTag}
              </Box> 
            </Box>;
          },
        },
        ...
        },
        {
          ...
          formatter: ({ row }: FormatterProps<R, unknown>) => {
            return <Box >{row.feeAmount} {row.feeSymbol}</Box>;
          },
        },
        {
          key: "Time",

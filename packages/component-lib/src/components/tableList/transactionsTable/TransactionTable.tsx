...
export const TransactionTable = withTranslation(['tables', 'common'])((props: TransactionTableProps & WithTranslation) => {
  ...
  const getColumnModeTransaction = React.useCallback((t: TFunction): Column<any, unknown>[] => [
    ...
    },
    {
      ...
      formatter: ({row}) => {
        const fee = row[ 'fee' ]
        const renderValue = `${getValuePrecisionThousand(fee.value, undefined, undefined, undefined, false, {
          floor: false,
          isTrade: true
        })} ${fee.unit}`
        return (
    ...
    },
    {
      ...
      formatter: ({row}) => {
        ...
        const formattedDetail = {
          ...
          from: senderAddress,
          to: receiver,
          fee: `${getValuePrecisionThousand(fee.value, undefined, undefined, undefined, false, {
            isTrade: true,
            floor: false
          })} ${fee.unit}`,
        ...
        return (
          <Box className="rdg-cell-value " display={'flex'} justifyContent={'flex-end'} alignItems={'center'}>
            {path ? <Link href={path}>
              ...
              <RenderValue onClick={() => handleTxnDetail(formattedDetail)}
                           title={value}>{value ? getFormattedHash(value) : EmptyValueTag}</RenderValue>}
            <Box marginLeft={1}>
              <CellStatus {...{row}} />
            </Box>
          </Box>

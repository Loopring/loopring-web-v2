...
export const TxnDetailPanel = withTranslation('common', {withRef: true})(
  React.forwardRef((
  {
    ...
    memo,
    etherscanBaseUrl,
  }: TxnDetailProps & WithTranslation,ref:React.ForwardedRef<any>) => {
    ...
    return <ContentWrapperStyled ref={ref} tabIndex={-1} >
      <HeaderStyled>
      ...
      <GridContainerStyled container flexDirection={'column'}>
        ...
        {txHash && <GridItemStyled item>
          ...
          <InfoValueStyled>
            <EthHshStyled color={'var(--color-secondary)'}
                          onClick={() => window.open(`${etherscanBaseUrl}tx/${txHash}`)}>{txHash}</EthHshStyled>

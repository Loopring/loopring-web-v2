import { Box, Checkbox, FormControlLabel as MuiFormControlLabel, Link, Typography } from '@mui/material';
    ...
    return <Box flex={1} display={'flex'} alignItems={'center'} justifyContent={'space-between'}
        ...
        <Box display={'flex'} flexDirection={'column'} justifyContent={'center'}
            ...
            <BoxStyle paddingX={5 / 3} display={'flex'} flexDirection={'row'}
                ...
                <MuiFormControlLabel
                    control={<CheckboxStyled className={isShake ? 'shake' : ''} checked={checkboxValue}
                                             onChange={handleCheckboxChange} checkedIcon={<CheckedIcon/>}
                                             icon={<CheckBoxIcon/>}
                                             color="default"/>}
                    label={<Trans i18nKey="labelProviderAgree">I have read, understand, and agree to the
                        <Link component={'a'}
                              href={termUrl}
                              onClick={(event) => {
                                  //@ts-ignore
                                  window?._iub?.badges['0'].linkA.dispatchEvent(new Event("click"))

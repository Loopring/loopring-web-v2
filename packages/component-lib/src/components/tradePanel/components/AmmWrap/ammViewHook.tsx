...
import { AccountStatus, EmptyValueTag, ReverseIcon } from '@loopring-web/common-resources'
    ...
    const stob = React.useMemo(() => {
        if (ammCalcData && ammCalcData?.lpCoinA && ammCalcData?.lpCoinB && ammCalcData.AtoB) {
            ...
            if (_isStoB) {
                ...
                price = `1 ${ammCalcData?.lpCoinB?.belong} \u2248 ${ammCalcData.BtoA} ${ammCalcData?.lpCoinA?.belong}`;
            }

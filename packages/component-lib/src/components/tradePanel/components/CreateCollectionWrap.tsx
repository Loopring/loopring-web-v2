...
import { Button, ImageUploadWrapper, IPFSSourceUpload, TextareaAutosizeStyled } from '../../basic-lib';
import { Trans, useTranslation } from 'react-i18next';
import React from 'react';
import { CreateCollectionViewProps, TradeBtnStatus } from '../Interface';
...
export const CreateCollectionWrap = <T extends Partial<CollectionMeta>>(
	...
	>
		...
		>
			...
			<Grid item xs={12} md={2} lg={2} position={"relative"} display={'flex'}
			...
			<Grid item xs={12} md={4} lg={3} position={"relative"} display={'flex'}
			      flexDirection={isMobile ? 'column' : 'row'} justifyContent={'space-between'}
			>
				<Box width={"100%"}>
					<Typography
			...
			<Grid item xs={12} md={6} lg={7} display={'flex'} flexDirection={'column'} position={"relative"}>
			...
			<Grid item xs={12}>
				...
				>
					{btnInfo ? t(btnInfo.label, btnInfo.params) : t(`labelCollectionCreateBtn`)}
				</Button>
			</Grid>
		</Grid>

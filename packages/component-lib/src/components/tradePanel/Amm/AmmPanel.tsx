...
export const AmmPanel = withTranslation('common', {withRef: true})(<T extends AmmJoinData<C extends IBData<I> ? C : IBData<I>>,
    ...
    const [ammChgWithdrawData, setAmmChgWithdrawData] = React.useState<AmmWithdrawChgData<TW>>({
    ...
    useEffect(() => {
        if (routerLocation) {
            ...
            const index = customType === 'remove' ? AmmPanelType.Exit : AmmPanelType.Join
            setIndex(index)
        }
    }, [routerLocation])
    //
    ...
    const _onChangeAddEvent = React.useCallback(async ({tradeData, type}: AmmChgData<T>) => {
        ...
        if (typeof onAmmAddChangeEvent == 'function') {
            setAmmChgDepositData(onAmmAddChangeEvent({tradeData, type} as AmmChgData<T>));
        } else {
    ...
    const _onChangeRemoveEvent = React.useCallback(async ({
        ...
        if (typeof onRemoveChangeEvent == 'function') {
            setAmmChgWithdrawData(onRemoveChangeEvent({tradeData, type:'lp'} as AmmWithdrawChgData<TW>));
        } else {
    ...
    const panelList: Pick<PanelContent<"ammJoin" | "ammExit">, 'key' | 'element'>[] = [
        ...
        {
            key: "ammExit",
            element: React.useMemo(() => <AmmWithdrawWrap<TW, I, ACD, C> key={"ammExit"} {...{
                t,
                ...rest,
                anchors,
                disableWithdraw,
